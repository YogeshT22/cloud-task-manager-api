...
@router.get(
    "/", response_model=List[schemas.TaskResponse], operation_id="get_all_tasks"
)
def get_tasks(
    db: Session = Depends(get_db),
    current_user: models.User = Depends(oauth2.get_current_user),
...
@router.delete(
    "/{id}", status_code=status.HTTP_204_NO_CONTENT, operation_id="delete_task"
)
def delete_task(
    id: int,
    db: Session = Depends(get_db),
...
@router.post(
    ...
    operation_id="send_task_reminder",
)
def send_reminder(
    id: int,
    db: Session = Depends(get_db),
    ...
    try:
        ...
        # this send_task_reminder is the function defined in tasks.py, and .delay() is a Celery method that queues the task for asynchronous execution by a worker.
